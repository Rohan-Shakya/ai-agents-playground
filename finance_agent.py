...
# Importing OpenAI and related libraries
from openai import OpenAI

# Importing necessary classes from langchain and other related tools
...
# Loading environment variables (API keys) from .env file
load_dotenv()
...
# Define custom typed dictionary for agent state and structured query outputs
from typing import TypedDict, List
from pydantic import BaseModel

# Define the AgentState data structure for holding the agent's state
...
# Queries object to structure search queries
...
# Function to gather financials by reading CSV file and sending the data to the model
...
# Function to research competitors and gather related data
...
# Function to compare performance between the company and its competitors
...
# Function to research critique feedback and generate responses
...
# Function to collect feedback from the user based on previous analysis
...
# Function to write the final report based on all the collected data
...
# Decision function to check if further revisions are needed
def should_continue(state):
    if state["revision_number"] > state["max_revisions"]:
        return END
    return "collect_feedback"

# Build the state graph for the entire process
builder = StateGraph(AgentState)

# Add all the defined nodes to the graph
...
# Add conditional edges based on whether the task should continue
...
# Define the edges between the nodes to create the flow
...
# Streamlit function to create the UI for interacting with the agent
def main():
